...
content_image_path = "images/1-content.png"
...
content_tensor = image2tensor(content_image).cuda()
style_tensor = image2tensor(style_image).cuda()
target_tensor = initial_target_tensor(content_tensor).cuda().requires_grad_()
...
vgg19_features_model = vgg19_model.features.cuda()
...
def calc_vgg19_features(vgg19_features_model, inputs):
    ...
    for idx, layer in vgg19_features_model._modules.items():
        ...
        if idx in style_layers:
            ...
            gram_matrix = gram_matrix / (_H * _W)   # normalize
            output_features["style"][name] = gram_matrix
...
def calc_content_loss(target_feature, content_feature):
    content_loss = mse_loss(target_feature, content_feature)
    return content_loss


def calc_style_loss(target_feature, style_feature):
    _C1, _C2 = target_feature.shape
    assert _C1 == _C2
    style_loss = mse_loss(target_feature, style_feature)
    style_loss /= (_C1 ** 2)
    return style_loss


def calc_tv_loss(target_tensor):
    x = target_tensor[:, :, 1:, :] - target_tensor[:, :, :-1, :]
    y = target_tensor[:, :, :, 1:] - target_tensor[:, :, :, :-1]
    tv_loss = torch.sum(torch.abs(x)) + torch.sum(torch.abs(y))
    return tv_loss
...
for iter_idx in range(num_iters):
    ...
    for name, weight in content_layers.items():
        content_loss += weight * calc_content_loss(target_features["content"][name], content_features["content"][name])
        break

    style_loss = 0
    for name, weight in style_layers.items():
        style_loss += weight * calc_style_loss(target_features["style"][name], style_features["style"][name])

    tv_loss = calc_tv_loss(target_tensor)
    ...
    if (iter_idx % num_iters_show == 0) or (iter_idx == num_iters):
        print("iter: %03d, loss : %.2f (content: %.2f, style: %.2f, TV: %.2f)" % (
            iter_idx, loss.item(), content_loss.item(), style_loss.item(), tv_loss.item()
        ))
        if preserve_color:
        ...
        cv2.imencode(".jpg", target_image)[1].tofile("test/iter_%03d.png" % iter_idx)
...
if preserve_color:
    ...
    target_image = tensor2image(target_tensor)
# show_image(target_image)
cv2.imencode(".jpg", target_image)[1].tofile("test/final.png")
