...
def saveimg(img, iters):
    if (PIXEL_CLIP=='True'):
        img = img.clip(0, 255)
    cv2.imencode(".jpg", img)[1].tofile("test/origin_iter_%03d.png" % iters)
...
vgg.load_state_dict(torch.load("C:/Users/Administrator/.cache/torch/checkpoints/vgg19-d01eb7cb-caffe.pth"), strict=False)
